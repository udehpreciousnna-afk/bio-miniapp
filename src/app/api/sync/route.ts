...
export async function POST(req: NextRequest) {
  ...
  try {
    await fetch(`${PHP_BASE}/api/sync_user.php`, {
      ...
      body: JSON.stringify({ user_id:telegramUser.id, telegram_name:`${telegramUser.first_name??''} ${telegramUser.last_name??''}`.trim(), username:telegramUser.username??'', wallet_address:'', bio_balance:0, eth_balance:0 }),
    })
    const r = await fetch(`${PHP_BASE}/api/me_by_id.php?uid=${telegramUser.id}`, { headers:{ 'X-Bot-Secret':PHP_SECRET } })
